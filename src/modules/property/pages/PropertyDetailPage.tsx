...
export function PropertyDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-background pb-28">
      ...
      {/* 2.5 LOCATION-FIRST ACCESS ENFORCEMENT */}
      {showLocationGate && (
      ...
      {!showLocationGate && locationAccess?.count === 0 && (
        <div className="mx-auto max-w-2xl px-6 py-20 text-center">
          <div className="mx-auto mb-3.5 inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-muted text-muted-foreground">
            <MapPin className="h-6 w-6" />
          </div>
          <h2 className="text-xl font-bold text-foreground">
            No properties are currently available in this location.
          </h2>
          <p className="mt-2 text-xs text-muted-foreground">
            Try selecting another locality or city from our verified inventory.
          </p>
          <button
            onClick={() => setShowLocationGate(true)}
            className="mt-4 inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-bold text-white shadow-md hover:bg-emerald-500 transition"
          >
      ...
      {!showLocationGate && locationAccess?.ok && (
        <>
          {/* Location Validated Banner */}
          <div className="bg-emerald-500/10 border-y border-emerald-500/20 px-4 py-2.5 sm:px-6 mb-4">
            <div className="mx-auto max-w-7xl flex items-center justify-between">
              <p className="text-xs font-bold text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5">
                ...
                {locationAccess.message ||
                  `Properties available in ${property.locality || property.city}, ${property.city}`}
              </p>
              <button
                onClick={() => setShowLocationGate(true)}
          ...
          <main className="mx-auto max-w-7xl px-4 sm:px-6 mt-2">
            ...
            <EmiCalculatorModal
              ...
              initialPrice={
                property.listing_type === "sale" ? property.price : property.price * 250
              }
            />
              ...
              <DialogContent className="sm:max-w-md">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-xs text-muted-foreground pt-1">
                    SEEDHA Properties delivers 100% direct owner listings with 0% brokerage. Sign in
                    or create an account to view verified phone and WhatsApp details.
                  </DialogDescription>
                </DialogHeader>
