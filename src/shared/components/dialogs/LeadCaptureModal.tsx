  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await logLiveActivity({
        ...
        contact_name: name,
        contact_phone: phone,
        activity_type: actionType === "schedule_visit" ? "schedule_visit" : "inquiry",
        ...
        {submitted ? (
          <div className="py-8 text-center space-y-4">
            ...
            <p className="text-xs text-muted-foreground leading-relaxed">
              Thank you, <strong className="text-foreground">{name}</strong>. Our Area Agent for <strong className="text-foreground">{locality}</strong> has been notified to assist you with viewing <strong className="text-foreground">{propertyTitle}</strong>.
            </p>
            <button
              onClick={onClose}
          ...
          <>
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-extrabold text-foreground text-base">
                  {actionType === "schedule_visit" ? "Schedule Site Visit" : "Unlock Owner Direct Contact"}
                </h3>
                <p className="text-[11px] text-muted-foreground">
                  Connect with owner for <strong className="text-foreground">{propertyTitle}</strong> ({locality})
                </p>
              </div>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5 text-xs">
              ...
              <div className="space-y-1">
                <label className="font-bold text-foreground flex items-center gap-1.5">
                  <Calendar className="h-3.5 w-3.5 text-primary" /> Preferred Move-in Date / Inquiry Note
                </label>
                <input
                  type="text"
              ...
              <div className="pt-1">
                ...
                >
                  ...
                  <span>{actionType === "schedule_visit" ? "Confirm Site Visit Request" : "Unlock Owner Contact & Details"}</span>
                </button>
              </div>

              <p className="text-[10px] text-muted-foreground text-center">
                🔒 0% Brokerage Guarantee. Your contact details are shared only with verified owners and area agents.
              </p>
            </form>
          </>
        )}
