...
export function DemoModeSwitcher() {
  ...
  const roles: { role: UserRole; label: string; icon: any; color: string }[] = [
    ...
    { role: "owner", label: "Property Owner", icon: Building, color: "text-emerald-500 bg-emerald-500/10" },
    { role: "agent", label: "Real Estate Agent", icon: UserCheck, color: "text-purple-500 bg-purple-500/10" },
    { role: "admin", label: "Platform Admin", icon: Shield, color: "text-amber-500 bg-amber-500/10" },
  ];
    ...
    <div className="fixed bottom-4 right-4 z-50 flex items-center gap-2">
      <div className="relative">
        {isOpen && (
          <div className="absolute bottom-12 right-0 mb-2 w-64 rounded-2xl border border-border bg-card/95 p-2 shadow-2xl backdrop-blur-xl">
            <div className="flex items-center justify-between px-3 py-2 border-b border-border/40">
              ...
              <span className="text-[10px] bg-secondary px-1.5 py-0.5 rounded text-muted-foreground">DEV MODE</span>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <div className={`grid h-6 w-6 place-items-center rounded-lg ${isSelected ? "bg-white/20 text-white" : color}`}>
        ...
        >
          ...
          <span>Demo Role: <span className="capitalize text-primary">{currentRole}</span></span>
