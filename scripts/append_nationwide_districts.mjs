#!/usr/bin/env node
/**
 * Ingests all remaining administrative districts and principal cities across all 34 other States & UTs.
 * Idempotent, safe, connects via DATABASE_URL to live Neon staging DB and updates the 006 migration file.
 */
...
// Comprehensive district lists for all 34 remaining States & UTs
const ALL_OTHER_DISTRICTS = {
  ...
  "in-as": [
    ...
    { name: "Cachar", hq: "Silchar", lat: 24.8333, lng: 92.7789, pin: "788001" },
  ...
  "in-cg": [
    ...
    { name: "Bastar", hq: "Jagdalpur", lat: 19.0740, lng: 82.0298, pin: "494001" },
  ...
  "in-od": [
    ...
    { name: "Mayurbhanj", hq: "Baripada", lat: 21.9333, lng: 86.7333, pin: "757001" },
  ...
  "in-pb": [
    ...
    { name: "SAS Nagar", hq: "Mohali", lat: 30.7046, lng: 76.7179, pin: "160055" },
    ...
    { name: "Fazilka", hq: "Fazilka", lat: 30.4037, lng: 74.0254, pin: "152123" },
...
async function main() {
  console.log("Applying nationwide administrative coverage to live database...");
  let insertedDistricts = 0;
  let insertedCities = 0;
  let insertedPins = 0;

  for (const [stateId, distList] of Object.entries(ALL_OTHER_DISTRICTS)) {
    const stateCode = stateId.replace("in-", "").toUpperCase();
    ...
    for (const d of distList) {
      ...
      // Insert District
      await sql`
        INSERT INTO public.locations (id, parent_id, country_code, type, name, normalized_name, state_id, district_id, city_id, state_code, district_code, city_code, pincode, latitude, longitude, status, source)
        VALUES (${distId}, ${stateId}, 'IN', 'DISTRICT', ${d.name}, ${d.name.toLowerCase().trim()}, ${stateId}, ${distId}, NULL, ${stateCode}, ${distSlug.toUpperCase()}, NULL, NULL, ${d.lat}, ${d.lng}, 'ACTIVE', 'GOI_LGD_CENSUS')
      ...
      await sql`
        INSERT INTO public.locations (id, parent_id, country_code, type, name, normalized_name, state_id, district_id, city_id, state_code, district_code, city_code, pincode, latitude, longitude, status, source)
        VALUES (${cityId}, ${distId}, 'IN', 'CITY', ${d.hq}, ${d.hq.toLowerCase().trim()}, ${stateId}, ${distId}, ${cityId}, ${stateCode}, ${distSlug.toUpperCase()}, ${cityId}, ${d.pin}, ${d.lat}, ${d.lng}, 'ACTIVE', 'GOI_LGD_CENSUS')
        ON CONFLICT (id) DO UPDATE SET
          name = EXCLUDED.name,
          normalized_name = EXCLUDED.normalized_name,
      ...
      // Insert Pincode
      if (d.pin) {
        const pinId = `in-pin-${d.pin}`;
        await sql`
          INSERT INTO public.locations (id, parent_id, country_code, type, name, normalized_name, state_id, district_id, city_id, state_code, district_code, city_code, pincode, latitude, longitude, status, source)
          VALUES (${pinId}, ${cityId}, 'IN', 'PINCODE', ${`${d.pin} - ${d.hq}`}, ${d.pin}, ${stateId}, ${distId}, ${cityId}, ${stateCode}, NULL, ${cityId}, ${d.pin}, ${d.lat}, ${d.lng}, 'ACTIVE', 'GOI_LGD_CENSUS')
          ON CONFLICT (id) DO UPDATE SET
  ...
  console.log(`Successfully inserted/updated ${insertedDistricts} districts, ${insertedCities} cities, ${insertedPins} pincodes.`);

  // Append SQL to 006 migration file
  const migrationPath = path.join("scripts", "migrations", "006_authoritative_india_locations_master.sql");
  let appendSql = "\n-- Additional Nationwide Administrative Coverage\n";
  ...
  for (const [stateId, distList] of Object.entries(ALL_OTHER_DISTRICTS)) {
    ...
    for (const d of distList) {
      ...
      appendSql += `INSERT INTO public.locations (id, parent_id, country_code, type, name, normalized_name, state_id, district_id, city_id, state_code, district_code, city_code, pincode, latitude, longitude, status, source)
VALUES ('${distId}', '${stateId}', 'IN', 'DISTRICT', '${d.name.replace(/'/g, "''")}', '${d.name.toLowerCase().trim().replace(/'/g, "''")}', '${stateId}', '${distId}', NULL, '${stateCode}', '${distSlug.toUpperCase()}', NULL, NULL, ${d.lat}, ${d.lng}, 'ACTIVE', 'GOI_LGD_CENSUS')
ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, normalized_name = EXCLUDED.normalized_name, updated_at = NOW();\n`;

      appendSql += `INSERT INTO public.locations (id, parent_id, country_code, type, name, normalized_name, state_id, district_id, city_id, state_code, district_code, city_code, pincode, latitude, longitude, status, source)
VALUES ('${cityId}', '${distId}', 'IN', 'CITY', '${d.hq.replace(/'/g, "''")}', '${d.hq.toLowerCase().trim().replace(/'/g, "''")}', '${stateId}', '${distId}', '${cityId}', '${stateCode}', '${distSlug.toUpperCase()}', '${cityId}', '${d.pin}', ${d.lat}, ${d.lng}, 'ACTIVE', 'GOI_LGD_CENSUS')
ON CONFLICT (id) DO UPDATE SET name = EXCLUDED.name, normalized_name = EXCLUDED.normalized_name, pincode = EXCLUDED.pincode, updated_at = NOW();\n`;

      appendSql += `INSERT INTO public.locations (id, parent_id, country_code, type, name, normalized_name, state_id, district_id, city_id, state_code, district_code, city_code, pincode, latitude, longitude, status, source)
VALUES ('${pinId}', '${cityId}', 'IN', 'PINCODE', '${d.pin} - ${d.hq.replace(/'/g, "''")}', '${d.pin}', '${stateId}', '${distId}', '${cityId}', '${stateCode}', NULL, '${cityId}', '${d.pin}', ${d.lat}, ${d.lng}, 'ACTIVE', 'GOI_LGD_CENSUS')
ON CONFLICT (id) DO UPDATE SET parent_id = EXCLUDED.parent_id, city_id = EXCLUDED.city_id, updated_at = NOW();\n`;
    }
  }

  fs.appendFileSync(migrationPath, appendSql, "utf8");
  console.log(`Appended nationwide statements to ${migrationPath}`);
}
